...
function AdminQuizzesList() {
  ...
  const quizzesQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error, count } = await q.order("created_at", { ascending: false }).limit(300);
      if (error) throw error;
    ...
    <PageContainer>
      ...
      {quizzesQuery.isLoading || unitsQuery.isLoading ? (
        ...
        <EmptyState
          icon={FlaskConical}
          title={
            search || subjectId !== "all" || status !== "all" ? "No matches" : "No quizzes yet"
          }
          description={
              ...
              {items.map((q) => {
                ...
                return (
                  <tr key={q.id} className="text-sm hover:bg-surface-muted/40">
                    ...
                    <td className="px-3 py-2.5">
                      <Badge variant="outline">{q.passing_pct}%</Badge>
                    </td>
                    <td className="px-3 py-2.5 hidden md:table-cell text-xs text-muted-foreground">
                    ...
                    <td className="px-3 py-2.5 hidden lg:table-cell text-xs text-muted-foreground">
                      {formatDistanceToNow(new Date(q.updated_at ?? q.created_at), {
                        addSuffix: true,
                      })}
                    </td>
                    <td className="px-3 py-2.5 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {q.status !== "published" ? (
                            <DropdownMenuItem
                              onClick={() => updateStatus.mutate({ id: q.id, status: "published" })}
                            >
                              <CheckCircle2 className="mr-2 h-3.5 w-3.5" /> Publish
                            </DropdownMenuItem>
                          ) : (
                            <DropdownMenuItem
                              onClick={() => updateStatus.mutate({ id: q.id, status: "draft" })}
                            >
